...
from models.model import RecurrentModel
from recurrent_embedding_methods import *
from recurrent_utils import get_dataloaders_from_dict, batch_correct, test_epoch
import pandas as pd


recurrent_types = ['RNN', 'LSTM', 'GRU']

for recur_type in recurrent_types:
    ...
    all_dataset_dicts = dataset_dictionaries1 #+ dataset_dictionaries2 + dataset_dictionaries3


    pandas_ls = []
    for data in all_dataset_dicts:
        ...
        model = RecurrentModel(input_size = inp_size, hidden_size = 2048, 
                                    num_layers = 3, num_classes = OUT_SIZE, device = device, type = recur_type)
        model.to(device)
        ...
        for epoch in range(1,101):
            ...
            for i, (input, target) in enumerate(train_loader):
                ...
                #print(input.shape)
                #input = input.reshape(-1, inp_size, 1)
                #print(input.shape)
                optimizer.zero_grad()
        ...
        df.to_csv('%s_results.csv'%recur_type)