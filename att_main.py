...
from models.classAttModel import ClassAtt
...
data = get_elmo()
_, e1 = get_dataloaders_from_dict(data, bs = 128)
data = get_unirep()
_, e2 = get_dataloaders_from_dict(data, bs = 128)
data = get_transformer()
_, e3 = get_dataloaders_from_dict(data, bs = 128)

torch.manual_seed(231)

M = 1850
model = ClassAtt(e1, e2, e3, M, 2048, OUT_SIZE)
model.to(device)

data = get_all_concat()
loaders, _ = get_dataloaders_from_dict(data, bs = 128)
