...
from models.classAttSelfModel import Transformer
...
dataset_dicts = [get_SVD_x(1000), get_SVD_x(1250), get_SVD_x(1500), get_SVD_x(1750), get_1toN_x(1000), get_1toN_x(1250), get_1toN_x(1500), get_1toN_x(1750),
                        get_elmo_and_transformer_concat(), get_elmo(), get_transformer(), get_elmo_and_transformer_inter_avg(),  get_elmo_and_transformer_padded_avg()]

pandas_ls = []
for data in dataset_dicts:

    loaders, emb_size = get_dataloaders_from_dict(data, bs = 1)
    ...
    model = Transformer(N, h, dff, d_model, proj_size, emb_size, out_size, device)
    model.to(device)
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4,  betas=(0.99, 0.98), eps=1e-09, weight_decay=1e-5)
    ...
    df = pd.DataFrame(data = pandas_ls, columns = ['train', 'val', 'fold', 'superfamily', 'family'])
    df.to_csv('results_selfatt.csv')